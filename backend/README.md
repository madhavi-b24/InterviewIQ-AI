...
| `interview.py` | `interview_sessions`, `interview_rounds` — planning fields (`requested_difficulty`/`starting_difficulty`/`mode`/`plan_snapshot`, nullable `resume_id`) added in Module 4, see Database.md §5; `questions`, `answers` wired in Module 5; `question_test_cases`, `code_submissions`, `code_submission_test_results` wired in Module 6, plus Module 6's additive `questions.coding_problem_id`/`coding_snapshot` and `code_submissions.error_message` columns |
| `coding.py` | *(Module 6)* `CodingProblem`/`CodingProblemTestCase` — the coding-problem catalog, mirrors `planning.py`'s shape exactly (seed-driven, snapshot-at-use-time). |
| `evaluation.py` | `answer_evaluations`; `coding_evaluations` wired in Module 6 with its additive `edge_case_score`/`edge_case_explanation`/`overall_code_score`/`strengths`/`weaknesses`/`recommendations` columns |
...
| `repositories/interview.py` | `InterviewSessionRepository` — `get_owned`/`list_owned` (Module 4) plus *(Module 5)* `get_owned_for_update` (row lock for concurrent-submission safety); `InterviewRoundRepository`, `QuestionRepository`, `AnswerRepository` (Module 5). *(Module 6)* `QuestionRepository.get_with_round` now also eager-loads `test_cases`; new `list_coding_problem_ids_for_session` (whole-interview repeat avoidance) and a new `QuestionTestCaseRepository`. |
...
| `repositories/coding.py` | *(Module 6)* `CodingProblemRepository` (`list_active_for_selection`, `get_active_with_test_cases`), `CodeSubmissionRepository` (`get_with_test_results`, `list_for_answer`, `get_final_for_answer`, `next_attempt_no`), `CodeSubmissionTestResultRepository`, `CodingEvaluationRepository`. |
...
| `services/interview/execution_service.py` | *(Module 5)* `InterviewExecutionService` — the execution use case: `start_interview`/`get_current_turn`/`submit_answer`/`abandon`. Same shape as `InterviewPlannerService` (constructor takes only `AsyncSession`, builds its own repositories). The only caller of `build_interview_graph()`; owns the two-phase transaction/locking/idempotency design — see "Interview Engine (Module 5)" below. *(Module 6)* Adds `complete_coding_round` (the `CODING_ROUND_COMPLETE` trigger's entry point) and a new `_persist_coding_question`/`_apply_round_transition_result` split — the latter factored out of `submit_answer` so both callers share the same round-advancement tail instead of duplicating it. `_first_executable_round_index`'s coding-round exclusion is gone (coding rounds are real now). |
...
| `services/coding/` | *(Module 6)* `catalog_seed.py` (idempotent upsert of `data/coding_problems.json`, mirrors `services/planning/catalog_seed.py`'s pattern) and `coding_round_service.py` (`CodingRoundService` — the Run/Submit use case: `create_submission`/`execute_and_grade_submission`/`get_submission`/`list_submissions_for_question`/`get_evaluation`; owns the two-phase persist-then-grade design and the `is_final`-release-on-infra-failure logic — see "Coding Round & Code Execution (Module 6)" below). |
| `services/code_evaluation/` | *(Module 6)* The coding-quality LLM seam, mirroring `services/interview_intelligence/` file-for-file: `provider.py` (`CodeEvaluationProvider` Protocol — one `evaluate()` method), `schemas.py` (`CodeEvaluationResult`, Gemini-schema-safe from day one), `gemini_provider.py`, `fake_provider.py`, `factories.py` (`CODE_EVALUATION_PROVIDER: Literal["gemini","fake"]`). |
...
| `jobs/background_tasks_runner.py` | MVP implementation, wrapping FastAPI's `BackgroundTasks`. `JOB_HANDLERS` — `process_resume` (Module 3), `run_code_submission` (Module 6, see below). Swapping to Celery later means writing `CeleryRunner` and flipping `JOB_RUNNER_BACKEND`, not touching any caller. |
...
| `jobs/coding_execution.py` | *(Module 6)* `run_code_submission_job` — the coding-round grading pipeline's background stage: builds its own `CodeExecutor`/`CodeEvaluationProvider` (outside FastAPI's DI, same reasoning as `resume_processing.py`), calls `CodingRoundService.execute_and_grade_submission`, then — only for a *final* submission that reached a genuine graded verdict — advances the interview via `InterviewExecutionService.complete_coding_round`. See "Coding Round & Code Execution (Module 6)" below. |
| `execution/base.py` | `CodeExecutor` protocol + `TestCase`/`TestCaseResult`/`ExecutionOutcome` dataclasses — `ExecutionOutcome` (Module 6) separates "did it compile" from per-test results, rather than encoding a compile failure into every `TestCaseResult`. |
| `execution/docker_sandbox.py` | *(Module 6)* `DockerSandboxExecutor` — despite the name, never touches the Docker API/socket; makes a plain HTTP call to the sibling `executor` container. Implemented for real in this module (was a `NotImplementedError` stub through Module 5). |
| `execution/fake_executor.py` | *(Module 6)* `FakeCodeExecutor` — deterministic, magic-marker-driven (`COMPILE_ERROR`/`RUNTIME_ERROR`/`TIME_LIMIT`/`OUTPUT_LIMIT`/`INCORRECT_SOLUTION` substrings in `source_code`), mirrors every other fake provider's singleton/`.reset()`/`.calls` shape. |
| `execution/factories.py` | *(Module 6)* `build_code_executor(settings)` — config-driven selection, same `_reject_fake_in_production` guard as every other factory. |
| `execution/exceptions.py` | *(Module 6)* `CodeExecutionError`/`CodeExecutionTimeoutError`/`CodeExecutionProviderError`. |

### `execution_worker/` — the sandbox executor, a SEPARATE deployable unit (Module 6)

Not under `app/` at all, deliberately — this is its own Docker image (`execution_worker/Dockerfile`, its own `docker-compose.yml` service), imports nothing from `app/`, and never receives application secrets or source. See "Coding Round & Code Execution (Module 6)" below for the full isolation-layer writeup.

| File | Purpose |
|---|---|
| `app.py` | The tiny FastAPI service itself — `GET /health`, `POST /execute`. Deliberately doesn't know about "expected output"/pass-fail (that comparison happens in the trusted `backend` container, `DockerSandboxExecutor`) — it just runs code against stdin and reports what happened. Hard-clamps every caller-supplied limit regardless of what's asked for. |
| `languages.py` | Server-controlled `LANGUAGE_CONFIGS` (python/java/cpp) — fixed compile/run argv lists per language; no code path anywhere builds a command from user input. |
| `sandbox_runner.py` | The actual isolation mechanics: `resource.setrlimit` (CPU/memory/process-count/output-size), wall-clock timeout via `subprocess` + process-group `SIGKILL`, a fresh `tempfile.TemporaryDirectory()` per execution deleted on every exit path. Extensively documented on exactly what is and isn't achieved at this layer — see the file's own docstring. |
| `requirements.txt` | `fastapi`+`uvicorn` only — no `app/` dependencies, no database driver, no LLM SDK. |
| `Dockerfile` | `python:3.12-slim` + `default-jdk-headless`/`g++`, non-root `sandboxrunner` user, `COPY`s only this directory's 3 files — no way for `backend/app/` source to end up in this image, by construction. |

### `app/agents/` — LangGraph wiring (Module 5)

| File | Purpose |
|---|---|
| `state.py` | `InterviewState` TypedDict — the graph's transient execution state (Architecture.md §5.2). Every field carries a one-line "why this exists" comment; rebuilt fresh from Postgres every turn, never trusted from a stale checkpoint for a durable fact. *(Module 6)* Adds the `CODING_ROUND_COMPLETE` trigger, `QuestionRef.coding_problem_id`, `role_key`, `coding_problem_candidates`/`coding_problem_history`, and a new `CodingProblemCandidate` TypedDict (deliberately excludes test-case data — hidden tests never enter graph state, let alone the LangGraph checkpointer). `rounds_to_skip` — the only thing Module 5's coding-round skip ever populated — is gone. |
| `policy.py` | Every deterministic, non-LLM decision rule in one place: `DEFAULT_QUESTIONS_PER_ROUND`, `MAX_FOLLOW_UPS_PER_QUESTION`, `compute_difficulty_signal`/`apply_difficulty_signal` (the adaptive-difficulty threshold policy). *(Module 6)* Adds `select_coding_problem` (deterministic problem-picking — difficulty match, role priority, whole-interview repeat avoidance) and `compute_overall_code_score` (the deterministic weighted aggregate). `CODING_ROUND_SKIP_REASON` is gone — nothing skips coding rounds anymore. |
...
| `graph.py` | `build_interview_graph(provider, checkpointer)` — compiles the real `StateGraph(InterviewState)`. *(Module 6)* Adds a `select_coding_problem` node and a `round_transition` entry point for the new `CODING_ROUND_COMPLETE` trigger — see "Coding Round & Code Execution (Module 6)" below for the full graph shape. |
| `nodes/supervisor.py` | Supervisor Agent — `route_entry` (trigger-based routing — *(Module 6)* now also round-type-aware for `START`, and routes `CODING_ROUND_COMPLETE` straight to `round_transition`), `decide_next_action` (the FOLLOW_UP/NEXT_QUESTION/NEXT_ROUND/COMPLETE decision, deterministic threshold logic over structured signals, never an LLM-chosen transition). |
| `nodes/round_management.py` | Round-transition mechanics split out from the Supervisor's decision — advances `round_plan` position by one. *(Module 6)* No longer skips `coding` rounds (that placeholder branch is gone, not merely disabled); `route_after_round_transition` is now a 3-way route (`generate_question`/`select_coding_problem`/`complete`). |
| `nodes/coding_problem_selector.py` | *(Module 6)* `select_coding_problem_node` — the coding round's equivalent of `question_generator.py`: picks exactly one problem, deterministically, from the small precomputed candidate list in state (never a DB query, never an LLM call). |
...
| File | Purpose |
|---|---|
| `deps.py` | Every shared FastAPI dependency: `DbSession`, `RedisClient`, `get_current_user`/`CurrentUser` (decodes a bearer access JWT, loads the `User` row), `require_role(*roles)` (403 role-authorization factory), `get_auth_service`/`get_email_provider`/`get_google_oauth_provider` (config-driven), `get_job_runner`, `get_resume_storage`/`get_resume_embedding_index`/`get_resume_service` (Module 3), `get_catalog_service`/`get_interview_planner_service` (Module 4), `get_interview_execution_service`/`get_interview_agent_provider` (Module 5 — the provider is eagerly constructed, unlike the resume embedding index, since `/start`/`/answers` genuinely cannot proceed without it; `InterviewIntelligenceProviderError` is translated to a clean `503`, not left to leak as an unhandled exception), *(Module 6)* `get_code_executor`/`CodeExecutorDep` (now actually calls `build_code_executor`, previously an unwired stub), `get_code_evaluation_provider`/`CodeEvaluationProviderDep`, `get_coding_round_service`/`CodingRoundServiceDep`, and `db_healthcheck`/`redis_healthcheck`. |
...
| `v1/interviews.py` | Interview session router (`/interview-sessions/*`) — planning (create/list/get/plan, Module 4) plus *(Module 5)* `/start`, `/current-turn`, `/answers`, `/abandon`. |
| `v1/code_submissions.py` | *(Module 6)* Coding-round router, same `/interview-sessions/{id}` prefix — `GET .../questions/{qid}/coding-problem`, `POST`/`GET .../questions/{qid}/code-submissions`, `GET .../code-submissions/{id}`, `GET .../code-submissions/{id}/evaluation`. Thin, same shape as every other router; imports `app.jobs.coding_execution` for its `JOB_HANDLERS` registration side-effect (mirrors `v1/resumes.py`'s import of `jobs.resume_processing`). |
...
| `versions/..._coding_round_execution.py` | Module 6 migration — additive only, four independent pieces: (1) new `coding_problems`/`coding_problem_test_cases` catalog tables; (2) `coding_evaluations` gains 6 columns (`edge_case_score`/`edge_case_explanation`/`overall_code_score`/`strengths`/`weaknesses`/`recommendations`), every one `server_default`ed since the table currently has zero rows; (3) `code_submissions.execution_status`'s Postgres enum gains 4 new values (`compile_error`/`runtime_error`/`memory_limit`/`output_limit`) via `ALTER TYPE ... ADD VALUE IF NOT EXISTS` — additive-only, no `DROP VALUE` counterpart, documented as a one-way step in `downgrade()`; (4) `questions` gains nullable `coding_problem_id`/`coding_snapshot`, and `code_submissions` gains nullable `error_message`. Verified `upgrade → downgrade -1 → upgrade head` and `alembic check` (no drift) on both `interviewiq` and `interviewiq_test`. |

### `tests/`

| File | Purpose |
|---|---|
| `conftest.py` | Loads `.env.test` *before* anything imports `app.core.config` (import order matters here — see the `noqa: E402`s), then provides an `httpx.AsyncClient` fixture wired to the app via `ASGITransport` (no real HTTP server needed for tests). Also: an autouse fixture that truncates `users` (cascading to `refresh_tokens`/`password_reset_tokens`/every `resume_*` table) before every test, resets the fake resume-intelligence singleton (Module 3), the fake interview-agent singleton (Module 5), *and (Module 6) the fake code executor/code-evaluation singletons*, and disposes the process-level DB engine after each test — see the fixture's docstring for why disposal is necessary on top of truncation (pytest-asyncio gives every test its own event loop; pooled asyncpg connections are bound to the loop that opened them). *(Module 6)* A second autouse fixture, `_seed_coding_catalog`, reseeds the coding-problem catalog before every test the same way `_seed_catalog` already does for the planning catalog. `FakeEmailProvider` — an in-memory `EmailProvider` double used via `app.dependency_overrides` so password-reset tests can read the raw reset token without it ever touching a log. |
...
| `test_interview_execution.py` | *(Module 5, updated Module 6)* Tests across start/answer/current-turn/abandon, follow-up generation and the follow-up cap, adaptive-difficulty thresholds (increase/decrease/maintain/clamping), round transitions, resume-grounded vs. generic question topics, ownership, no-internal-reasoning-exposed, and LLM-failure handling (provider failure preserves the answer, retry doesn't duplicate; timeout returns a clean error). Uses `FakeInterviewAgentProvider` (deterministic, no network) — never depends on live Gemini for the automated suite. An autouse `_ensure_checkpoint_tables` fixture calls `AsyncPostgresSaver.setup()` before every test in this file, since httpx's `ASGITransport` (used by the `client` fixture) never triggers FastAPI's lifespan, which is where this normally happens for a real deployment. *(Module 6)* Two tests superseded, not just left stale: `test_all_coding_plan_rejected_at_start` → `test_all_coding_plan_starts_with_a_coding_question` (a coding-only plan now starts successfully instead of `422 ALL_ROUNDS_UNSUPPORTED`), `test_coding_round_is_skipped_not_faked` → `test_coding_round_in_mixed_plan_is_reached_not_skipped` (a mixed plan's coding round is now reached and marked `active`, not silently `skipped`). |
| `test_coding_round.py` | *(Module 6)* Functional coverage for the Run/Submit lifecycle: coding-problem retrieval (sample-only, hidden data never exposed), Run-vs-Submit's mandatory distinction (sample-only execution + no LLM call vs. all-tests + evaluation), multi-attempt preservation, final-submission-once (idempotent replay on the happy path, a genuine DB-constraint race forced via a monkeypatched pre-check on the concurrency path), failure handling (a compile error is a final graded verdict with no LLM call spent; a genuine sandbox/evaluation-provider infra failure releases the final slot for retry, a candidate-caused outcome never does), ownership, unsupported-language rejection, and the graph-integration path (a graded final submission completes both the round and — for a single-round plan — the whole interview; a Run never advances anything). Uses `FakeCodeExecutor`/`FakeCodeEvaluationProvider` (`CODE_EXECUTION_BACKEND=fake`/`CODE_EVALUATION_PROVIDER=fake` in `.env.test`) — never depends on the real sandbox or live Gemini for this file. |
| `test_sandbox_security.py` | *(Module 6)* The one place this suite talks to real infrastructure instead of a fake — see "Coding Round & Code Execution (Module 6)" below for the full account of what was and wasn't verified this way, and why it manages its own throwaway Docker container/network rather than reusing the compose-managed `executor` service. |
...
### Coding-round boundary — superseded by Module 6

This subsection originally documented that a `coding` round in the plan was never executed, only cleanly skipped (`round_transition_node` marking `InterviewRound.status=skipped`) — Module 5's deliberate scope boundary. **Coding rounds execute for real as of Module 6** — see "Coding Round & Code Execution (Module 6)" below for the full design and verification writeup. The Module 5 skip behavior no longer exists in the code (not merely disabled); this note is kept only so the history is honest rather than silently rewritten.
...
## Coding Round & Code Execution (Module 6)

### Architecture

```
GET .../coding-problem, POST/GET .../code-submissions, GET .../code-submissions/{id}[/evaluation]
  → app/api/v1/code_submissions.py (thin — parses request, calls one CodingRoundService
    method, shapes response)
    → CodingRoundService (app/services/coding/coding_round_service.py) — owns the
      Run/Submit use case: ownership checks, the two-phase persist-then-grade split,
      the is_final-release-on-infra-failure logic
        → JobRunner.enqueue("run_code_submission", ...) — Phase 1 ends here; the
          request returns 202 before any execution happens
        → (background) app/jobs/coding_execution.py::run_code_submission_job —
          builds its own CodeExecutor/CodeEvaluationProvider (no request-scoped DI
          in a background task), calls back into CodingRoundService for the actual
          execute-score-evaluate pipeline, then — final + genuinely graded only —
          calls InterviewExecutionService.complete_coding_round to advance the round
            → DockerSandboxExecutor (app/execution/) — one HTTP call to the sibling
              `executor` container, never Docker directly
            → CodeEvaluationProvider (app/services/code_evaluation/) — Gemini
              structured-output call, final submissions only

Reaching a coding round in the first place (picking WHICH problem) goes through the
SAME LangGraph engine as a text round, not a separate mechanism:
  InterviewExecutionService.start_interview / submit_answer / complete_coding_round
    → build_interview_graph(...).ainvoke(state) → select_coding_problem_node
      (deterministic, no DB access, no LLM call) → InterviewExecutionService
      snapshots the picked catalog problem into Question + question_test_cases rows
```

Run/Submit are deliberately **outside** the graph — a different shape entirely (async, polled, multi-attempt) from a synchronous one-shot text-answer turn, and nothing about executing/grading code needs the Supervisor's turn-taking machinery. Only two things touch the graph: which problem a round presents (`select_coding_problem_node`), and telling the graph a coding round is done so it can advance (`CODING_ROUND_COMPLETE` trigger). See Architecture.md §5.4's "As implemented" note for the full reasoning.

### Graph shape (extending Module 5's)
...
```
    START --(route_entry: trigger, current_round)-->
        ...
        "SUBMIT_ANSWER"             -> evaluate_answer -> ... -> decide_next_action
                                       --(route_next_action)--> ... NEXT_ROUND -> round_transition
                                           --(route_after_round_transition)-->
                                               "generate_question"     -> ... -> END
                                               "select_coding_problem" -> ... -> END
                                               "complete"              -> ... -> END
        "CODING_ROUND_COMPLETE"     -> round_transition --(same 3-way route as above)-->
```

`select_coding_problem_node` is the coding round's `question_generator_node` equivalent — same "id assigned after persistence" placeholder convention, same DB-free/LLM-free node discipline, but picks from a small precomputed candidate list (`InterviewState.coding_problem_candidates` — every active catalog problem, deliberately excluding test-case data even from graph state) via `app/agents/policy.py::select_coding_problem`: exact difficulty match, then whole-interview repeat avoidance (`coding_problem_history`), then role-priority. `CODING_ROUND_COMPLETE` never touches `evaluate_answer`/`adapt_difficulty` — a coding round has no free-text answer or difficulty signal of its own; it goes straight to the *same, unmodified* `round_transition` node the `NEXT_ROUND` path already uses, and both callers now share `InterviewExecutionService._apply_round_transition_result` for the round-advancement tail instead of duplicating it.

### Sandbox isolation model

Resolves Architecture.md §6's originally-deferred choice ("Docker-outside-of-Docker via socket mount" vs. "a small sibling `execution-worker` service") as: **a dedicated sibling `executor` container, socket-mount ruled out categorically** — the task's prohibition list treats Docker-socket access as absolute for every component, not just the one running candidate code, and zero components having it is the only reading defensible without qualification.

Four layers, each documented for exactly what it does and doesn't guarantee (never overclaimed):

1. **Network boundary** (strong, Docker/kernel-enforced): `executor` sits on `docker-compose.yml`'s `sandbox_internal` network, `internal: true` — Docker refuses to route it anywhere outside the Compose project. No app-level flag to misconfigure; this can't be bypassed from inside a sandboxed subprocess by any means.
2. **Container hardening** (also kernel-enforced): `read_only: true` root filesystem, a size-capped `tmpfs` at `/tmp` as the only writable path, `cap_drop: [ALL]`, `security_opt: [no-new-privileges:true]`, a non-root `sandboxrunner` user baked into the image. No `env_file`, no secret env vars, no project source, no `.env` ever reach this container — its own image (`execution_worker/`) imports nothing from `app/`.
3. **Per-execution OS limits** (process-level — the layer honestly *not* container-per-execution): `resource.setrlimit` for CPU/address-space/process-count, a wall-clock `subprocess` timeout that SIGKILLs the whole process group, and `RLIMIT_FSIZE` on redirected output *files* (not "capture then truncate," which would let an output bomb exhaust the executor's own memory first) for the byte cap. A fresh `tempfile.TemporaryDirectory()` per execution, deleted on every exit path via the context manager.
4. **Documented limitation**: concurrent submissions share the `executor` container's kernel/process table — process isolation *within* one locked-down container, not a fresh container/VM per run. `RLIMIT_NPROC` is a fork-bomb mitigation shared across the whole container's process budget (server + every sandboxed subprocess run as one uid), not a hard per-execution guarantee.

`backend`'s `DockerSandboxExecutor` never sends `executor` a test case's expected output — only `id` + `input` (as stdin). `executor` reports back exactly what happened (stdout/stderr/exit code/timing/timeout/truncation flags) and nothing else; the pass/fail comparison happens entirely on the trusted `backend` side, so a hidden test's answer never needs to cross into the sandbox boundary at all.

### Run vs. Submit lifecycle

Both call `CodingRoundService.create_submission`, which durably persists the `CodeSubmission` row (`QUEUED`) and enqueues `run_code_submission` **before any execution happens** — the same two-phase, commit-before-expensive-work discipline `submit_answer` established in Module 5. The endpoint returns `202` immediately with that pre-grading state; a real client polls `GET .../code-submissions/{id}` for the result (the response body is serialized before FastAPI's `BackgroundTasks` run the job, so even under this project's synchronous test harness the POST response itself never reflects grading — the test suite's `_submit_and_grade` helper always does one follow-up GET, documented in `tests/test_coding_round.py`'s module docstring).

- `is_final=false` ("Run") — executes only the question's *sample* test cases, never calls `CodeEvaluationProvider`. Fast, free, unlimited attempts.
- `is_final=true` ("Submit") — executes *every* test case (sample + hidden). If it compiles and runs, `correctness_score` is computed (weighted by `question_test_cases.weight`) and, unless it was a compile error (no code behavior to judge — no LLM call spent), `CodeEvaluationProvider.evaluate()` judges readability/optimization/edge-case handling. `overall_code_score` is then a deterministic weighted combination (`app/agents/policy.py::compute_overall_code_score` — correctness 0.40, optimization 0.20, readability 0.15, edge_case 0.15, quality 0.10 reusing readability) — never LLM-set, same "don't let the model pick the headline number" principle Module 5's difficulty policy uses.

Execution status is derived from structured signals the sandbox reports (`timed_out`/`exit_code`/`output_truncated` on `TestCaseResult`, `compiled`/`compile_message` on the whole-submission `ExecutionOutcome`) — never by string-matching `stderr` — with a documented precedence when multiple test cases disagree: compile error > timeout > output-limit > runtime-error > success/partial, so the candidate sees the single most informative status.

### Idempotency, concurrency, and the `is_final`-release design

The DB's `ux_code_submissions_final` partial unique index (`(answer_id) WHERE is_final`, Module 1 baseline, not reopened here) is the real backstop for "only one final submission per question," not just the Python-level pre-check — `CodingRoundService.create_submission` catches the `IntegrityError` a genuine race produces and replays the winning row instead of erroring. Verified by a dedicated regression test that forces the pre-check to lie (always report "no final exists yet") so the constraint itself is what's actually being exercised, not the pre-check — `tests/test_coding_round.py::test_concurrent_final_submissions_hit_the_db_constraint_backstop`.

**`is_final` is not simply write-once.** It's released back to `false` — but *only* when grading never reached a genuine verdict because OUR infrastructure failed (the sandbox was unreachable, or the evaluation LLM call failed) — freeing the unique slot so a retry is possible. A candidate outcome that genuinely ran (success, partial, compile error, runtime error, timeout, output-limit) is always final and graded; only a bare `execution_status=error` (infra, never candidate-caused) ever triggers the release. Getting this distinction right was the actual design difficulty here: setting `is_final=true` immediately (a durable reservation, matching `submit_answer`'s two-phase pattern) is what makes concurrent-Submit protection work at all, but never releasing it on ANY failure would mean a transient sandbox outage permanently locks a candidate out of ever submitting again for a question that was never actually graded.

A real bug surfaced by the test suite while building this, not by inspection: the `IntegrityError` handler's `await self._session.rollback()` expires every ORM object in the session, and the code immediately re-read `answer.id` synchronously afterward — an expired-attribute access that needs an implicit refresh, which is unsafe under the async engine (`MissingGreenlet`, the exact same class of bug `app/services/coding/catalog_seed.py`'s module docstring already documents once for a different trigger). Fixed by capturing `answer_id` as a plain value before any code path that could roll back, never re-reading it off the ORM object afterward.

### Coding-problem catalog

`coding_problems`/`coding_problem_test_cases` (new tables, seeded from `app/services/coding/data/coding_problems.json` via `catalog_seed.py`'s idempotent upsert) — a deliberately **small, hand-verified 8-problem catalog** spanning every topic area the module asked for (arrays, strings, hash maps, binary search, stacks/queues, trees, graphs, dynamic programming) across easy/medium/hard, rather than hundreds of generated ones. Every problem is a whole stdin/stdout program (not a function-call harness), including a from-scratch explicit-array tree representation for the tree problem specifically, to avoid ambiguous BFS-serialization parsing across three languages. **Every test case's expected output was independently verified against a from-scratch reference solution before being committed** — not hand-typed and trusted — see the module's implementation notes for the verification script.

Selecting a problem for a round **snapshots** it into a live `Question` (`question_type=coding`, `coding_problem_id` FK for traceability, `coding_snapshot` jsonb for the immutable content copy) plus one `QuestionTestCase` row per catalog test case — the same "catalog is mutable, live instances are immutable copies" rule Module 4's `plan_snapshot` already established, so editing the catalog later never changes a problem a candidate has already been asked. Hidden (`is_sample=false`) test cases never leave this snapshot boundary through any API response — `GET .../coding-problem` and `GET .../code-submissions/{id}` both filter to `is_sample=true` only; hidden outcomes are folded into aggregate pass/fail counts alone.

### Languages supported — Python, Java, C++ (a documented deviation)

Features.md's original MVP language list said "Python + JavaScript"; this module's task explicitly specified **Python, Java, C++** instead. Followed the explicit instruction for this module and corrected both Features.md and Roadmap.md rather than silently diverging from the stale doc. Compile/run commands come from a fixed, server-side dict (`execution_worker/languages.py::LANGUAGE_CONFIGS`) keyed only by language name — no code path anywhere constructs a command line from request content, which is what "commands must come from server-controlled configuration" means structurally here, not just by convention.

### Known limitations

- Concurrent submissions share the `executor` container's process table (documented in the isolation-model section above) — not a fresh container/VM per execution. A genuine MVP trade-off, not an oversight.
- `RLIMIT_NPROC` is a shared-uid fork-bomb mitigation, not a hard per-execution process-count guarantee (same section).
- `code_submissions.peak_memory_kb` is never populated — the sandbox enforces a memory *limit* but does not currently measure and report peak usage per execution.
- `code_submissions.executor` (an audit/debugging column for which backend actually ran a submission) is never populated — the executor identity is implied by config, not written per-row.
...
## Manually verifying the Coding Round with Swagger

Requires the full stack including `executor` (`docker compose up -d` from the repo root — `uv run uvicorn app.main:app --reload` alone won't have a sandbox to talk to unless `CODE_EXECUTION_BACKEND=fake`).

1. Create a plan using the **"Coding Practice"** template (`role_key=software_engineer`, mode `coding_only`) — the shortest path to a coding question (its one round). Copy the interview `id`.
2. **Start it**: `POST /api/v1/interview-sessions/{id}/start` → Execute. Expect `200` with `current_round: "coding"` and a `question` object whose `round_type` is `"coding"`. Copy `question.id`.
3. **Fetch the problem**: `GET /api/v1/interview-sessions/{id}/questions/{question_id}/coding-problem` → Execute. Expect `200` with `title`/`description`/`difficulty`/`constraints`/`starter_code`/`supported_languages` and a non-empty `sample_test_cases` array — confirm no field resembling a hidden test's input/expected_output appears anywhere in the response.
4. **Run against sample cases**: `POST /api/v1/interview-sessions/{id}/questions/{question_id}/code-submissions` → body `{"language": "python", "source_code": "<a correct solution for the problem you were given>", "is_final": false}` → Execute. Expect `202` with `is_final: false`, `attempt_no: 1`, `execution_status: "queued"`.
5. **Poll for the Run's result**: `GET /api/v1/interview-sessions/{id}/code-submissions/{submission_id}` (the `id` from step 4) → Execute. Expect `execution_status: "success"`, `total_test_count` equal to exactly the sample-case count from step 3 (never more — hidden tests never ran), and `sample_test_results` populated with real `actual_output`/`passed` per case.
6. **Confirm no evaluation exists yet**: `GET .../code-submissions/{submission_id}/evaluation` → Execute. Expect `null` — a Run never triggers code-quality grading.
7. **Submit for real**: repeat step 4 with `"is_final": true`. Expect a fresh `202`, `attempt_no: 2`.
8. **Poll for the Submit's result**: repeat step 5 against the new submission `id`. Expect `total_test_count` now **greater than** the sample-case count (hidden tests ran too), `graded_at` set once grading finishes, and `sample_test_results` still itemizing only the sample subset.
9. **Fetch the evaluation**: `GET .../code-submissions/{submission_id}/evaluation` → Execute. Expect a full `CodingEvaluationOut` — `correctness_score`/`overall_code_score` numeric and consistent with the pass rate, `readability_explanation`/`optimization_explanation`/`edge_case_explanation` genuine prose (not templated filler if a real Gemini key is configured), `strengths`/`weaknesses`/`recommendations` non-generic.
10. **Confirm round/interview advanced**: `GET /api/v1/interview-sessions/{id}` → Execute. Expect `status: "completed"` (Coding Practice has only this one round).
11. **Resubmission-once check**: repeat step 4 with `"is_final": true` again, different source code. Expect the **same** submission `id` as step 7 back (idempotent replay), not a new attempt and not an error.
12. **Ownership check**: register a second account, Authorize with *its* token, then repeat steps 3/4/5 using the first account's `interview_id`/`question_id`/`submission_id` → expect `404` on all three.
...
## What was actually verified, not just written (Module 6)

Every piece of this milestone was run, not just authored — including against the real sandbox, not only the fake:

- `uv sync` — no new main dependencies (`httpx` was already pinned; the sandbox executor is a fully separate deployable with its own minimal `requirements.txt`, not part of `backend`'s dependency graph at all).
- New migration (`coding round execution`) autogenerated, hand-adjusted (server defaults on every new NOT NULL column, explicit additive enum-value handling, named FK constraints), round-tripped `upgrade → downgrade -1 → upgrade head` and `alembic check` (no drift) on **both** `interviewiq` and `interviewiq_test` — twice, in fact: the migration was amended mid-module (two more nullable columns added, `questions.coding_problem_id`/`coding_snapshot` and `code_submissions.error_message`) while still uncommitted, and re-verified clean after each amendment, including a full DB reset on `interviewiq_test` to guarantee no stale schema state from an earlier version of the same not-yet-committed migration.
- `ruff check .` and `black --check .` — clean across every new/changed file, backend and `execution_worker/` both.
- `pytest tests/test_coding_round.py` (fake `CodeExecutor`/`CodeEvaluationProvider`, no real sandbox or live Gemini needed) — **16 passed, 0 failed**, covering coding-problem retrieval (hidden data never exposed), Run-vs-Submit's mandatory distinction, multi-attempt preservation, final-submission-once (idempotent replay + a genuinely forced DB-constraint race), failure handling (compile error = final graded verdict with no LLM spend; genuine infra failure releases the final slot; a candidate outcome never does), ownership, unsupported-language rejection, and graph-integration (a graded final submission completes the round *and* the interview).
- **Full suite, every file, including the real-sandbox security tests in the same run (Docker was up) — 171 passed, 0 failed** (143 prior Module 1–5 tests + 16 new Module 6 functional tests + 12 real-sandbox security tests), confirming zero regression to auth/resume/planning/interview-execution alongside full Module 6 coverage, fake-based and real-sandbox-based together in one consolidated pass.
- **Found and fixed three real bugs via the automated suite, not by inspection:**
  1. `CodingRoundService.create_submission`'s `IntegrityError`-catch path called `await self._session.rollback()`, which expires every ORM object in the session, then immediately read `answer.id` synchronously — an expired-attribute access needing an implicit refresh, unsafe under the async engine (`MissingGreenlet`). Fixed by capturing `answer_id` as a plain value before the insert attempt, never re-reading it off the ORM object afterward.
  2. `question.test_cases` and `submission.test_results`/`.answer`/`.test_case` were read by the API-response-shaping code without ever being eager-loaded by the repository methods that fetched those rows — three separate `MissingGreenlet` crashes, one per relationship, each caught by a different functional test actually exercising the response path (`GET .../coding-problem`, `GET .../code-submissions`, `POST .../code-submissions`). Fixed by adding `selectinload()` for each relationship to the repository methods that feed these endpoints (`QuestionRepository.get_with_round`, `CodeSubmissionRepository.get_with_test_results`/`list_for_answer`).
  3. `catalog_seed.py`'s idempotent upsert accessed `problem.test_cases` (a relationship) immediately after `session.add(problem); await session.flush()` on a brand-new object — flushing does not populate an unloaded collection relationship the way a fresh in-memory object's collection normally would, and the ORM attempted a genuine lazy-load, hitting the same `MissingGreenlet` class of bug. Fixed by replacing the relationship access with an explicit `select(CodingProblemTestCase).where(...)` query, mirroring `app/services/planning/catalog_seed.py::_upsert_rounds`'s existing pattern for exactly this reason.
- Docker build of the standalone `executor` image (`docker build ./execution_worker`) — succeeds (~2 minutes cold, mostly `apt-get install default-jdk-headless g++`).
- **Real sandbox security suite (`tests/test_sandbox_security.py`, 12 tests) run against the actual running `executor` container over Docker — not mocked, not the fake executor.** First attempt at the container-lifecycle harness genuinely failed and taught something real: publishing a port (`-p host:8100`) on a Docker network created with `--internal` never becomes reachable from the host — `docker run` succeeds, uvicorn logs it's listening, but the port is simply never dialable, because an `internal: true` network has no gateway/NAT infrastructure at all for a published port to route through, regardless of whether the container itself is listening. Redesigned the harness to talk to the container via `docker exec ... python3` instead (hitting `localhost:8100` from inside the container's own namespace) — which is arguably *more* faithful to production than a port-published test container would have been, since the real `executor` service publishes no port either.
  - **Found and fixed a second harness bug** while getting this working: the embedded exec-script mixed `print()` (text-mode, buffered) with `sys.stdout.buffer.write()` (binary, unbuffered) for the status-line/body split, and the two streams flushed in the wrong order, corrupting every response. Fixed by routing every write through the same `sys.stdout.buffer` stream.
  - **Found and fixed a real product bug in `execution_worker/sandbox_runner.py`**, not a test bug: `output_truncated` was computed as `len(raw_stdout) > max_output_bytes`, but `RLIMIT_FSIZE` makes the captured file physically incapable of exceeding that limit — so the flag could never fire. Confirmed the actual mechanism by hitting it directly: a real output-bomb submission returns `exit_code: 1` (a normal-looking Python exit, not a raw signal termination) with `stderr` containing `OSError: [Errno 27] File too large` — buffered stdio surfaces the kernel's `RLIMIT_FSIZE` enforcement as an uncaught Python exception, not a `SIGXFSZ`-terminated process, so neither the exit code nor the file size alone revealed truncation the way the original `>` comparison assumed. Fixed by changing the comparison to `>=` (a file sized exactly at the limit is the real, practical signal — a legitimate program's output landing on that exact byte boundary is not a realistic candidate-code scenario). Re-verified the specific test (`test_output_bomb_is_capped_not_unbounded`) goes from failing to passing with the fix, against the real container both times.
  - Also fixed one genuine test-assertion bug the same run surfaced: `test_no_application_source_or_secrets_are_present` originally asserted `/app` doesn't exist — but `/app` is the *executor service's own* `WORKDIR` (its own `app.py`/`languages.py`/`sandbox_runner.py`), a naming coincidence with the unrelated main-backend `app/` package, not a leak. Narrowed the assertion to backend-specific markers (`/app/alembic`, `/app/pyproject.toml`, `/app/app/core`) that would only exist if the main backend's source were mistakenly mounted — confirmed absent — while the environment-variable check (only `HOME`/`LANG`/`PATH` ever present, no `GEMINI_API_KEY`/`DATABASE_URL`/etc.) was correct from the start.
  - **Final state: all 12 security tests pass against the real container**: infinite loop killed by the real wall-clock timeout; a real outbound connection attempt genuinely blocked (confirmed via the real errno, see the live smoke test below); no backend source or secrets present, only `HOME`/`LANG`/`PATH` in the environment; writing outside `/tmp` blocked by the real read-only root filesystem; no Docker socket file present; the process runs as a real non-root uid and `os.setuid(0)` is genuinely blocked; a 2 GiB allocation attempt hits the real memory limit and fails cleanly, not the host; an output bomb is genuinely capped (post-fix); a fork-bomb attempt is bounded and the container stays responsive afterward; shell metacharacters embedded in submitted source print completely literally — no shell ever interprets them; an unsupported language is rejected with `422` before anything executes; and a marker file written by one execution is confirmed absent in a completely fresh execution's scratch directory afterward.
- `docker compose up -d --build` (full stack: postgres, redis, chromadb, backend, **executor**) — builds and starts cleanly; `executor` reports `(healthy)`; `GET /api/v1/health` → `{"status":"ok","checks":{"database":true,"redis":true}}`.
- Catalog seeding: `coding_problems` was empty in the (until now unseeded, for this new table) dev database, same expected situation Module 4 documented for its own catalog. Extended the existing `uv run python -m app.db.seed_catalog` CLI (rather than adding a second, easy-to-forget command) to seed both catalogs in one step — ran it against the live container: `Interview catalog seeded: {companies: 6, roles: 7, templates: 10, rounds: 29}` / `Coding catalog seeded: {problems: 8, test_cases: 23}`.
- **Live end-to-end smoke test, real Docker sandbox AND real Gemini API, against the full running `docker compose` stack — not mocked, not simulated, `CODE_EXECUTION_BACKEND=docker_sandbox`/`CODE_EVALUATION_PROVIDER=gemini` (the actual production defaults, not the test-suite's fake overrides):**
  - Planned + started a "Coding Practice" interview → landed directly on a real coding question (`current_round: "coding"`), catalog-selected problem "Maximum Level Sum in a Binary Tree", one sample test case returned, no hidden test data anywhere in the response.
  - **Run** with a correct hand-written solution → real sandbox execution → `success`, 1/1 sample tests passed, no evaluation triggered (confirmed `GET .../evaluation` returns `null`).
  - **Run** with a deliberately wrong solution → real sandbox execution → `partial`, 0 passed, `attempt_no: 2` (both attempts preserved, neither overwritten).
  - **Submit** the correct solution as final → real sandbox execution against **2** test cases (sample + a genuinely hidden one — confirmed by `total_test_count` growing from 1 to 2 between Run and Submit) → `success`, then **real Gemini evaluation** completed moments later (`graded_at` populated) with genuinely specific, evidence-citing feedback quoting the actual submitted algorithm ("Uses an optimal Breadth-First Search (BFS) algorithm...", a concrete suggestion to use `float('-inf')` instead of `None` for the initial best-sum value) — not template filler. `correctness_score: 100.0`, `overall_code_score: 96.75` (the deterministic weighted combination, not an LLM-picked number).
  - Confirmed the round and the whole interview genuinely completed (`status: "completed"`) via the real `CODING_ROUND_COMPLETE` graph trigger — not asserted, read back from a fresh `GET /interview-sessions/{id}`.
  - **Idempotent resubmission**: submitting again (different, wrong source code) after the final attempt returned the *exact same* submission id — no second row created, no error.
  - **Two-candidate isolation, live**: a second registered candidate got a real `404` fetching the first candidate's problem; that second candidate's own freshly-planned interview got a genuinely different `interview_id`/`question_id`.
  - **Real infinite-loop timeout**: submitted `while True: pass` as candidate B's Run — the real sandbox killed it after **11.0 seconds** wall-clock (the configured 10s timeout plus overhead), `execution_status: "timeout"` — not simulated, an actual process genuinely running and genuinely killed.
  - **Real network-block, live**: submitted code attempting `socket.create_connection(('8.8.8.8', 53))` — the real sandbox returned the genuine OS-level error `[Errno 101] Network is unreachable`, proving the `internal: true` network boundary blocks real egress attempts made through the actual application flow, not just the standalone security-test harness.
  - **Real resource-cleanup verification**: after this entire session's activity (12 security tests plus the full live smoke sequence above — Runs, Submits, a real timeout kill, a real network-block attempt), `docker compose exec executor ls /tmp/` showed **zero leftover scratch directories** — every `tempfile.TemporaryDirectory()` was genuinely cleaned up on every exit path, including the timeout-kill path. Process count inside the container measured stable (5 before, 5 after) across a fresh benign execution — no leaked candidate process.
  - Unsupported language (`ruby`) on a real request → clean `422 UNSUPPORTED_LANGUAGE`, nothing executed.
